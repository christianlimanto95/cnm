...
$(function() {
    ...
    $(document).on("click", ".select", function(e) {
        ...
        if (selectedDropdown.hasClass("show") && !$(this).hasClass("select-autocomplete")) {
    ...
    $(document).on("click", ".dropdown-item", function(e) {
        ...
        if (select.hasClass("select-autocomplete")) {
    ...
    $(document).on("input", ".select-text", function(e) {
        var value = this.value.toLowerCase();
        var dropdownContainer = $(this).siblings(".dropdown-container");
        var dropdownItem = dropdownContainer.find(".dropdown-item");
        dropdownItem.each(function() {
            if (this.innerHTML.toLowerCase().indexOf(value) == -1) {
                $(this).addClass("hide");
            } else {
    ...
    $(document).on("keydown", ".select-text", function(e) {
        if (e.which == 40) {
            var dropdownContainer = $(this).siblings(".dropdown-container");
            var dropdownItemHover = dropdownContainer.find(".dropdown-item.hover");
            if (dropdownItemHover.length > 0) {
                dropdownItemHover.removeClass("hover");
            }

            var selectedItem = null;
            var next = dropdownItemHover.eq(0).next(".dropdown-item");
            if (next.length > 0) {
                next.addClass("hover");
                selectedItem = next;
            } else {
                var first = dropdownContainer.find(".dropdown-item").eq(0);
                if (first.length > 0) {
    ...
    $(".header-btn-apply-filter").on("click", function(e) {
    ...
    $(document).on("click", function(e) {
        ...
        if (target.closest(".header-search-container").length == 0) {
            $(".header-search-container").removeClass("active");
        }
    });
...
function setPagination(this_url, page, page_count, query) {
    if (query == null) {
        query = "";
    }
    var page_element = "";
    ...
    if (page - 2 > 1) {
        href = "href='" + this_url + "?page=1" + query + "'";
        page_element += "<a class='page' data-page='1' " + href + ">1</a>";

        if (page - 2 > 2) {
            page_element += "<a class='page-dot'>...</a>";
        }
    }

    for (var i = page - 2; i < page; i++) {
        if (i < 1) continue;
        href = "href='" + this_url + "?page=" + i + query + "'";
        page_element += "<a class='page' data-page='" + i + "' " + href + ">" + i + "</a>";
    }

    page_element += "<a class='page active' data-page='" + page + "' >" + page + "</a>";

    for (var i = page + 1; i <= page + 2 && i <= page_count; i++) {
        href = "href='" + this_url + "?page=" + i + query + "'";
        page_element += "<a class='page' data-page='" + i + "' " + href + ">" + i + "</a>";
    }
    ...
    if (page + 2 < page_count) {
        ...
        href = "href='" + this_url + "?page=" + page_count + query + "'";
        page_element += "<a class='page' data-page='" + page_count + "' " + href + ">" + page_count + "</a>";
    }

    $(".page-number-container").html(page_element);

    var page_next = $(".page-next"), page_prev = $(".page-prev");
    if (page == page_count) {
        page_next.addClass("disabled").removeAttr("href");
    } else {
        page_next.removeClass("disabled");
        page_next.attr("href", this_url + "?page=" + (page + 1) + query);
    }

    if (page == 1) {
        page_prev.addClass("disabled").removeAttr("href");
    } else {
        page_prev.removeClass("disabled");
        page_prev.attr("href", this_url + "?page=" + (page - 1) + query);
    }
}
...
function clearDialogInput(element) {
    element.find("input, textarea").val("");
    deleteImage(element.find(".form-right"));
}
